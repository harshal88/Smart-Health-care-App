...
public class DbAdapter {
	...
	public static final String K_TRIP_PURPOSE = "purp";
	...
	public static final String K_TRIP_LAT_HIGH = "lathi";
	public static final String K_TRIP_LAT_LOW = "latlo";
	public static final String K_TRIP_LGT_HIGH = "lgthi";
	public static final String K_TRIP_LGT_LOW = "lgtlo";
	public static final String K_TRIP_STATUS = "status";
	// table coords column names
	...
	private static final String TABLE_CREATE_TRIPS = "create table trips "
			+ "(_id integer primary key autoincrement, purp text, start long, endtime long, "
	...
	public boolean addCoordToTrip(long tripid, RunningPoint pt) {
		boolean success = true;

		// Add the latest point
		ContentValues rowValues = new ContentValues();
		rowValues.put(K_POINT_TRIP, tripid);
		rowValues.put(K_POINT_LAT, pt.latLng.latitude);
		rowValues.put(K_POINT_LGT, pt.latLng.longitude);
		...
		pt.speed = pt.speed * spdConvert;
		rowValues.put(K_POINT_SPEED, pt.speed);

		success = mDb.insert(DATA_TABLE_COORDS, null, rowValues) > 0;
	...
	public Cursor fetchCoordsForTrip(long tripid) {
		try {
			Cursor mCursor = mDb.query(true, DATA_TABLE_COORDS, new String[] {
					K_POINT_LAT, K_POINT_LGT }, K_POINT_TRIP + "=" + tripid,
					null, null, null, K_POINT_TIME, null);
	 ...
	 * indicate failure.
	 */
	public long createTrip(String purpose, double starttime, String fancystart,
			String note) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(K_TRIP_PURPOSE, purpose);
	...
	public Cursor fetchAllTrips() {
		Cursor c = mDb.query(DATA_TABLE_TRIPS, new String[] { K_TRIP_ROWID,
				K_TRIP_PURPOSE, K_TRIP_START, K_TRIP_FANCYSTART, K_TRIP_NOTE,
				K_TRIP_FANCYINFO }, null, null, null, null, "-" + K_TRIP_START);
	...
	public Cursor fetchTrip(long rowId) throws SQLException {
		Cursor mCursor = mDb.query(true, DATA_TABLE_TRIPS, new String[] {
				K_TRIP_ROWID, K_TRIP_PURPOSE, K_TRIP_START, K_TRIP_FANCYSTART,
				K_TRIP_NOTE, K_TRIP_LAT_HIGH, K_TRIP_LAT_LOW, K_TRIP_LGT_HIGH,
				K_TRIP_LGT_LOW, K_TRIP_STATUS, K_TRIP_END, K_TRIP_FANCYINFO,
				K_TRIP_DISTANCE },
	...
	public boolean updateTrip(long tripid, String purp, double starttime,
			String fancystart, String fancyinfo, String note, double lathigh,
			double latlow, double lgthigh, double lgtlow, float distance) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(K_TRIP_PURPOSE, purp);
		initialValues.put(K_TRIP_START, starttime);
		initialValues.put(K_TRIP_FANCYSTART, fancystart);
		initialValues.put(K_TRIP_NOTE, note);
		initialValues.put(K_TRIP_LAT_HIGH, lathigh);
		initialValues.put(K_TRIP_LAT_LOW, latlow);
		initialValues.put(K_TRIP_LGT_HIGH, lgthigh);
		initialValues.put(K_TRIP_LGT_LOW, lgtlow);
	...
	// //////my added function to get the purpose

	public ArrayList<String> getTripsPurposes() {

		Cursor cursor = mDbHelper.getReadableDatabase().rawQuery(
				"SELECT distinct (purp) FROM trips", null);
		cursor.moveToFirst();
		ArrayList<String> names = new ArrayList<String>();
		while (!cursor.isAfterLast()) {
			names.add(cursor.getString(cursor.getColumnIndex("purp")));
			cursor.moveToNext();
		}
		cursor.close();
	...
	public Cursor fetchporpouse(long rowId) throws SQLException {
		Cursor mCursor = mDb.query(true, DATA_TABLE_TRIPS,
				new String[] { K_TRIP_PURPOSE },

				K_TRIP_ROWID + "=" + rowId, null, null, null, null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
		}
		return mCursor;
